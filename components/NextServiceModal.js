...
import { Calendar, X, Save, Gauge, Clock } from 'lucide-react'

export default function NextServiceModal({ 
  isOpen, 
  onClose, 
  onCreateNextService, 
  vehicle,
  ...
  const [formData, setFormData] = useState({
    plannedDate: nextServiceData?.plannedDate || '',
    plannedTime: nextServiceData?.plannedDate ? new Date(nextServiceData.plannedDate).toTimeString().slice(0,5) : '09:00',
    duration: nextServiceData?.duration || 60,
    serviceType: nextServiceData?.serviceType || '',
    nextKm: nextServiceData?.nextKm || '',
    notes: nextServiceData?.notes || ''
  })
  const [loading, setLoading] = useState(false)

  // Update form data when nextServiceData prop changes
  useState(() => {
    if (nextServiceData) {
      setFormData({
        plannedDate: nextServiceData.plannedDate || '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.plannedDate || !formData.serviceType) {
      alert('Lütfen tarih ve servis tipi girin')
      return
    }
    ...
    try {
      ...
      console.error('Next service creation error:', error)
      alert('Sonraki servis oluşturulurken hata oluştu')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div>
            ...
            {nextServiceData && (
              <p className="text-sm text-green-600 mt-1">
                Formdan gelen bilgilerle önceden dolduruldu
              </p>
            )}
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
              placeholder="Sonraki servis hakkında ek notlar..."
            />
          </div>
